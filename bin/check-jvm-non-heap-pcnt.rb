...
#    ./check-jvm-non-heap-pcnt.rb -w 90 -c 95'
#
# http://www.rubydoc.info/gems/jolokia/0.1.0
# *:type=Connector,*

class CheckJvmNonHeapMemory < Sensu::Plugin::Check::CLI
  option :url,
  ...
  def run
    ...
    if pct_committed > config[:crit].to_i
      critical "Java Non-HEAP Memory: Committed #{pct_committed}%, Used #{pct_used}%"
    elsif pct_committed > config[:warn].to_i
      warning "Java Non-HEAP Memory: Committed #{pct_committed}%, Used #{pct_used}%"
    else
      ok "Java Non-HEAP Memory: Committed #{pct_committed}%, Used #{pct_used}%"
    end
